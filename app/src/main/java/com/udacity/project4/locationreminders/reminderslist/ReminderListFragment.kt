...
class ReminderListFragment : BaseFragment() {
                ...
                AuthUI.getInstance().signOut(requireContext())
                    .addOnCompleteListener {
                        if (it.isSuccessful) {
                            ...
                            Snackbar.make(requireView(), "Logout failed!", Snackbar.LENGTH_LONG)
                                .show()
                        }
                    }
                return true
    ...
    private fun observeAuthenticationState(){
        binding.viewModel!!.authenticationState.observe(viewLifecycleOwner, Observer { authenticationState ->
            when(authenticationState){
                AuthenticationState.LOGGED_IN -> Log.i("aut", "user logged in")
                AuthenticationState.LOGGED_OUT -> {
                    Log.i("aut", "user logged in")
                    startLoginProccess()
                }
            }
        })
    ...
    private fun startLoginProccess() {
